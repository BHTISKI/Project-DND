...
import { createInitialDeck, useGameStore } from './store';
import {
  setupMockRandom,
  resetMockRandom,
} from '../testUtils/mockRandom';
import { calculateUpgradeCost, enhanceEffect, shuffle } from '../utils/game';
import { loadMetaState, saveMetaState } from './persistence';
import { chooseArchetype, createEnemy, generateEnemyIntent } from '../engine/enemyArchetypes';
import { addStatus, statusValue } from '../engine/statuses';
import { generateAvailableNodes } from '../engine/runMap';
import { getRandomRewards } from '../engine/rewards';
...
describe('Store helper functions', () => {
  ...
  describe('enhanceEffect', () => {
    ...
    it('should enhance damage effect', () => {
      const effect: CardEffect = { kind: 'damage', amount: 4, damageBonus: 2 };
      const enhanced = enhanceEffect(effect);
      expect(enhanced).toEqual({ kind: 'damage', amount: 4, damageBonus: 4 });
    });
    ...
    it('should enhance another block value', () => {
      const effect: CardEffect = { kind: 'block', amount: 4 };
      const enhanced = enhanceEffect(effect);
      expect(enhanced).toEqual({ kind: 'block', amount: 6 });
    });
    ...
    it('should enhance another heal value', () => {
      const effect: CardEffect = { kind: 'heal', amount: 3 };
      const enhanced = enhanceEffect(effect);
      expect(enhanced).toEqual({ kind: 'heal', amount: 5 });
    });
  ...
  describe('generateEnemyIntent', () => {
    it('should generate attack intent based on weights', () => {
      ...
      const enemy = { id: 'e1', isim: 'Enemy', mevcutCan: 5, maksimumCan: 5, hasarBonusu: 1, ...initialPosture('goblin') };
    ...
    it('should generate special heal intent for mage', () => {
      ...
      const enemy = { id: 'e1', isim: 'Enemy', mevcutCan: 5, maksimumCan: 5, hasarBonusu: 1, ...initialPosture('mage') };
    ...
    it('should generate special heal intent for guardian', () => {
      setupMockRandom([0.9]); // high roll -> special
      const enemy = { id: 'e1', isim: 'Enemy', mevcutCan: 5, maksimumCan: 5, hasarBonusu: 1, ...initialPosture('guardian') };
...
describe('Store actions', () => {
  ...
  describe('endTurn', () => {
    it('should process enemy attack and reduce player HP', () => {
      // Fix the published attack independently of catalog size and shuffle calls.
      setupMockRandom([0.5]);
      useGameStore.getState().initializeGame();
      ...
      const attackLogExists = stateAfter.battleLogs.some(log => /Düşman \d+ hasar vurdu/.test(log));
    ...
    it('should transition to victory when enemy dies', () => {
      ...
      useGameStore.getState().endTurn();
      const stateAfter = useGameStore.getState();
      // Enemy should die from the poison step during turn resolution.
    ...
    it('should transition to gameOver when player dies', () => {
      // Keep intent selection deterministic; the published fixed attack is lethal at 1 HP.
      setupMockRandom([0.5]);
      useGameStore.getState().initializeGame();
      useGameStore.setState({
        ...useGameStore.getState(),
        ...
        enemy: { ...useGameStore.getState().enemy, mevcutCan: 10, hasarBonusu: 1}, // deterministic damage bonus
      });
      useGameStore.getState().endTurn();
      const stateAfter = useGameStore.getState();
  ...
  describe('playCard', () => {
    it('should play an attack card and reduce enemy HP', () => {
      ...
      setupMockRandom([0.5, 0.5]); // keep setup and intent selection deterministic
      useGameStore.getState().initializeGame();
      useGameStore.setState({
        ...useGameStore.getState(),
        gamePhase: 'combat',
        ...
        hand: [{ id: 'card-1', isim: 'Test Strike', tip: 'saldırı', manaBedeli: 1, baseHasar: 0,  effects: [{ kind: 'attack', amount: 3 }] }],
      });
      const stateBefore = useGameStore.getState();
  ...
  describe('removeCardFromDeck (shop action)', () => {
    it('should remove a card from deck when enough gold', () => {
      ...
      useGameStore.setState({
        ...useGameStore.getState(),
        gamePhase: 'shop',
        deck: [{ id: 'card-1', isim: 'Test Card', tip: 'saldırı', manaBedeli: 1, baseHasar: 0,  effects: [] }],
        gold: 60,
      });
      const stateBefore = useGameStore.getState();
  ...
  describe('upgradeCard (shop action)', () => {
    it('should upgrade a card when enough gold', () => {
      ...
      useGameStore.setState({
        ...useGameStore.getState(),
        gamePhase: 'shop',
        deck: [{ id: 'card-1', isim: 'Test Card', tip: 'saldırı', manaBedeli: 1, baseHasar: 0,  effects: [], rarity: 'common' }],
        gold: 60,
        victoryCount: 0,
      });
      const stateBefore = useGameStore.getState();
    ...
    it('should not upgrade card when not enough gold', () => {
      ...
      useGameStore.setState({
        ...useGameStore.getState(),
        gamePhase: 'shop',
        deck: [{ id: 'card-1', isim: 'Test Card', tip: 'saldırı', manaBedeli: 1, baseHasar: 0,  effects: [], rarity: 'legendary' }],
        gold: 50,
        victoryCount: 0,
      });
      const stateBefore = useGameStore.getState();
