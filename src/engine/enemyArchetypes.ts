...
export const enemyArchetypes: Record<EnemyArchetypeId, EnemyArchetypeConfig> = {
  goblin: { name: 'Goblin', hp: 7, ac: 11, power: 1, attackDamage: 4, block: 2, special: 'weakened', weights: [0.65, 0.2, 0.15], icon: 'G', visualTheme: 'goblin', role: 'Fırsatçı yağmacı', cardCount: 5 },
  guardian: { name: 'Muhafız', hp: 11, ac: 13, power: 0, attackDamage: 5, block: 4, special: 'heal', weights: [0.3, 0.55, 0.15], icon: '◆', visualTheme: 'guardian', role: 'Siper savaşçısı', cardCount: 6 },
  mage: { name: 'Büyücü', hp: 8, ac: 10, power: 2, attackDamage: 3, block: 2, special: 'damage', weights: [0.35, 0.15, 0.5], icon: '✦', visualTheme: 'mage', role: 'Kader dokuyucusu', cardCount: 5 },
  assassin: { name: 'Gölge Suikastçısı', hp: 6, ac: 12, power: 2, attackDamage: 5, block: 1, special: 'weakened', weights: [0.5, 0.1, 0.4], icon: '☠', visualTheme: 'assassin', role: 'Zehirli pusucu', cardCount: 4 },
  knight: { name: 'Kara Şövalye', hp: 14, ac: 14, power: 1, attackDamage: 6, block: 5, special: 'heal', weights: [0.35, 0.5, 0.15], icon: '♜', visualTheme: 'knight', role: 'Karşılık ustası', cardCount: 7 },
};
...
export function generateEnemyIntent(enemy: Character, archetypeId: EnemyArchetypeId, previous?: EnemyIntent | null): { intent: EnemyIntent; value: number; block: number } {
  ...
  if (archetype.special === 'heal') return { intent: { type: 'special', estimatedHeal: 4, effectKey: 'heal' }, value: 4, block: 0 };
  if (archetype.special === 'damage') return { intent: { type: 'special', estimatedDamage: 6 + enemy.gucCarpani, effectKey: 'arcane-blast' }, value: 6 + enemy.gucCarpani, block: 0 };
